...
for path in hd.ls(args.file):
    newpath = '{0}/ROOTCuts_output/ROOTCuts.txt'.format(path)
    print(newpath)
    file_list.append(newpath)

for f in file_list:
    with hd.open(f) as file:
        old_df_temp = pd.read_csv(file, delimiter=r'\s+')
...
for index, row in df_sig_masses.iterrows():
    ...
    df_NBJet = df.loc[((df['NBJet'] ==2) | (df['NBJet'] == 3))]
...
dict = {'MHT_eff': {'column': MHT_eff_tot, 'title': 'Fraction of Events with Missing-$H_{T} > 200$GeV', 'scale': vred, 'vmin': 0.5},
        ...
        'NBJet_eff': {'column': NBJet_eff_tot, 'title': 'Fraction of Events with $2 \leq$ \# of $b$-jets $\\leq 3$', 'scale': vlin, 'vmin': 0.},
        'BDP_eff': {'column': BDP_eff_tot, 'title': 'Fraction of Events with $\\Delta\\Phi^{*}$ > 0.5', 'scale': vlin, 'vmin': 0.},
...
for var in vars:
    ...
    cbar.set_label(dict[var]['title'], rotation=90, fontsize=8, labelpad=5)
    if not args.NoX:
        plt.show()
